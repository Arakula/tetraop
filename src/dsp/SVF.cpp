...
void SVF::setup(float _srate, float _freq, float _q, float resfactor)
{
	...
	g = std::tan(juce::MathConstants<float>::pi * std::fmin(freq / srate, 0.49f));
	r2 = (1.0f / q) * resfactor;
...
void SVF::processBlock(float* buf, int nsamples, int, int, float tfreq, float tq, float tgain, bool /*audioRate*/)
{
	if (mode == SVF::Off)
		return;
...
{
	float g_eval = std::tan(juce::MathConstants<float>::pi * std::fmin(_freq / srate, 0.49f));
